...
namespace canopen{
    ...
    void homing(std::string deviceFile, std::chrono::milliseconds syncInterval)
    {
        ...
        for (auto device : devices)
        {
            ...
            canopen::sendSDO((uint16_t)device.second.getCANid(), canopen::MODES_OF_OPERATION, canopen::MODES_OF_OPERATION_HOMING_MODE);
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
            canopen::sendSDO((uint16_t)device.second.getCANid(), canopen::CONTROLWORD, (uint16_t) (canopen::CONTROLWORD_ENABLE_OPERATION | canopen::CONTROLWORD_START_HOMING));
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
	...
	void init(std::string deviceFile, std::chrono::milliseconds syncInterval){
        ...
        if(!homing_active)
        {
            ...
            for (auto device : devices)
            {
                ...
                canopen::sendNMT((uint16_t)device.second.getCANid(), canopen::NMT_RESET_NODE);
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
                canopen::sendNMT((uint16_t)device.second.getCANid(), canopen::NMT_ENTER_PRE_OPERATIONAL);
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
                canopen::sendNMT((uint16_t)device.second.getCANid(), canopen::NMT_START_REMOTE_NODE);
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
	...
	void deviceManager() {
		...
		while (true) {
            ...
            if (!recover_active  | !atFirstInit | !homing_active)
            {
